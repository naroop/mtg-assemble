...
async function importDeck(deckId: string, text: string) {
  ...
  const fetchedCards = await Cards.collection(...parsedDeck.map((card) => ({ name: card.name })));

  const cardsPayload = parsedDeck.map((card) => {
    const scryfallCard = fetchedCards.find((fetchedCard) => fetchedCard.name.startsWith(card.name));
